...
- `主机号全为0` 网络本身, 如 `202.98.174.0`

- `主机号全为1` 广播地址, 如 `202.98.174.255`

- `127.0.0.0` 环回自检地址, 代表主机本身
...
`网络层协议` `分布式链路状态算法` `内部网关协议`

![报文格式](./assets/internet/OSPF_format.png)

- `洪泛法` 只有链路状态发生变化, 才向邻近路由器发送消息, 邻近路由器再将该消息转发给自己邻近的路由器
